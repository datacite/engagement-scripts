...
def get_doc_row(doc, category, parentDoc=""):
    doc_row = {i: doc[i] for i in doc if i != "details" and i != "children"}
    doc_row["categoryName"] = category["title"]
    if parentDoc:
        doc_row["parentDocName"] = parentDoc["title"]

    if parentDoc:
        doc_row["sortId"] = "#{:02d}.{:02d}.{:02d}".format(category["order"], parentDoc["order"], doc["order"])
    else:
...
def format_docs(docs_by_category):
